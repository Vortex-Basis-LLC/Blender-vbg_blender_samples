...
def do_animation_imports_exports_main(export_set: ExportSet):
    ...
    for fbx_path in export_set.fbxs:
        process_fbx(export_set, fbx_path, target_armature)
...
def process_fbx(export_set: ExportSet, fbx_path, target_armature):
    ...
    for source_armature in source_armature_list:
        ...
        add_copy_transforms_constraints_to_all_bones(source_armature, target_armature, calculate_initial_offsets(source_armature, target_armature))
...
def get_fbx_dirs(folder_path: str, name: str, loop: bool = False) -> Generator[ExportSet, None, None]:
    for dir_name in os.listdir(folder_path):
        ...
        if os.path.isdir(full_path):
            ...
            if regular_fbx:
                yield ExportSet(output_name, loop, regular_fbx)
            if root_motion_fbx:
                yield ExportSet(f"{output_name}_root", loop, root_motion_fbx)
            yield from get_fbx_dirs(full_path, f"{name}_{dir_name.lower()}", loop)
